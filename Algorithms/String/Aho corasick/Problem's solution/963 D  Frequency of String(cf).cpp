...
void Insert(string pat,int len, int p_id)
{
	...
	pat_end[now] = p_id;// p_id th  patt kon node e shes hoiche tar node 
...
void Search(string text)
{
	...
	for(int i=0;i<len;i++)
	{
		 ...
		 while(temp)
		 {
		 	index[pat_end[temp]].push_back(i+1);
...
int main()
{
	...
	for(int i=0;i<m;i++)
	{
		...
		for(int j=sz;j<index[i+1].size();j++)
		{
			int gp=index[i+1][j]-index[i+1][j-sz] + len;
			ans = min(gp,ans);	
